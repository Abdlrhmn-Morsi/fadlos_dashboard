...
const ProductForm = () => {
    ...
    const canUpdateProduct = hasPermission(Permissions.PRODUCTS_UPDATE);
    const canModifyProduct = isEditMode ? canUpdateProduct : canCreateProduct;

    // Permissions inheritance logic:
    // 1. If user has explicit variant permissions, use them strictly.
    // 2. If user has NO explicit variant permissions, inherit from product CRUD permissions.
    const employeePermissions = (user as any)?.employeeRole?.permissions || [];
    const hasExplicitVariantPermissions = employeePermissions.some((p: string) => p.startsWith('variants.'));

    const canViewVariants = hasPermission(Permissions.VARIANTS_VIEW);

    const canCreateVariants = hasExplicitVariantPermissions
        ? hasPermission(Permissions.VARIANTS_CREATE)
        : canCreateProduct;

    const canUpdateVariants = hasExplicitVariantPermissions
        ? hasPermission(Permissions.VARIANTS_UPDATE)
        : canUpdateProduct;

    const canDeleteVariants = hasExplicitVariantPermissions
        ? hasPermission(Permissions.VARIANTS_DELETE)
        : canModifyProduct;
                                        ...
                                        <div key={vIdx} className="p-4 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                                            <div className="space-y-4 mb-6">
                                                ...
                                                <div className="flex flex-wrap items-center gap-4">
                                                    <div className={clsx("flex-1 min-w-[140px] flex items-center justify-between px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg", !canModifyVariants && "opacity-70")}>
                                                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('isColor')}</span>
                                                        <label className={clsx("relative inline-flex items-center", canModifyVariants ? "cursor-pointer" : "cursor-default")}>
                                                            <input
                                                                type="checkbox"
                                            ...
                                            <div className="border-indigo-200 dark:border-indigo-900 space-y-4 ps-4 border-s-2">
                                                {variant.values.map((val, valIdx) => (
                                                    <div key={valIdx} className="bg-white dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-3">
                                                                ...
                                                                <div className="w-[50px] relative">
                                                                    ...
                                                                    >
                                                                        <input
                                                                            ...
                                                                            className={clsx("absolute inset-0 w-full h-full opacity-0", canModifyVariants ? "cursor-pointer" : "cursor-default")}
                                                                            disabled={!canModifyVariants}
                                                                        />
                                                                    </div>
                                                                </div>
                                                        ...
                                                        {/* Row 2: Inventory */}
                                                        <div className={clsx("flex items-center gap-4 bg-slate-50 dark:bg-slate-900/50 p-2 rounded-lg border border-slate-100 dark:border-slate-800", !canModifyVariants && "opacity-70")}>
                                                            <label className={clsx("flex items-center gap-2 text-xs font-medium text-slate-600 dark:text-slate-400", canModifyVariants ? "cursor-pointer" : "cursor-default")}>
                                                                <input
                                                                    type="checkbox"
                                                            ...
                                                            <div className={clsx("flex items-center gap-2 transition-opacity", !val.trackInventory && "opacity-50")}>
                                                                ...
                                                                <input
                                                                    ...
                                                                    placeholder="0"
                                                                    disabled={!val.trackInventory || !canModifyVariants}
                                                                />
                                                            </div>
                                                        </div>
                                                    </div>
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6">
                                ...
                                <InputGroup label={t('coverImage')} subtitle={t('coverImageSubtitle')} required>
                                    <div className="flex items-start gap-4">
                                        {(currCoverImage || coverImageFile) ? (
                                            <div className={clsx("relative group w-40 h-40 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700", !canModifyProduct && "pointer-events-none")}>
                                                <img
                            ...
                            <InputGroup label={t('galleryImages')} subtitle={t('galleryImagesSubtitle')}>
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    ...
                                    {/* Upload Button */}
                                    {canModifyProduct && existingImages.length + newImages.length < 8 && (
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <label className="flex items-center gap-2 cursor-pointer group">
                                    ...
                                    <div className={clsx("relative inline-flex items-center", canModifyProduct ? "cursor-pointer" : "cursor-default opacity-50")}>
                                        <input
                                            type="checkbox"
                                            className="sr-only peer"
                                        ...
                                        .map(a => {
                                            ...
                                            return (
                                                <div
                                                    key={a.id}
                                                    onClick={() => {
                                                        if (isDisabled || !canModifyProduct) return;
                                                        const newIds = isSelected
                                                    ...
                                                    className={clsx(
                                                        ...
                                                        (isDisabled || !canModifyProduct) ? "opacity-50 cursor-not-allowed" : "cursor-pointer group",
                                                        isSelected && !isDisabled
                                                            ? "bg-indigo-50 border-indigo-200 dark:bg-indigo-900/20 dark:border-indigo-800"
                                                            : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800",
                                                        !isDisabled && !isSelected && canModifyProduct && "hover:border-indigo-300"
                                                    )}
                                                >
                                        ...
                                        <div className="space-y-3">
                                            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 border border-dashed border-slate-300 dark:border-slate-700 rounded-lg text-center">
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => setIsCategoryModalOpen(true)}
                                                    className="w-full py-2 bg-indigo-600 text-white text-sm font-semibold rounded-lg hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-50"
                                                    disabled={!canModifyProduct}
                                                >
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between py-2">
                                    ...
                                    <label className={clsx("relative inline-flex items-center", canModifyProduct ? "cursor-pointer" : "cursor-default opacity-50")}>
                                        <input
                                            type="checkbox"
                                            className="sr-only peer"
