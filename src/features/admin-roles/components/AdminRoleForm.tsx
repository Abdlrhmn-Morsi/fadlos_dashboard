...
import { AdminRolesApiService } from '../api/admin-roles.api';
import { AdminPermissionGroup } from '../../../types/admin-permissions';
...
const AdminRoleForm = () => {
    ...
    const [permissionsData, setPermissionsData] = useState<{ categories: string[]; permissions: AdminPermissionGroup[] } | null>(null);
    ...
    useEffect(() => {
        const data = AdminRolesApiService.getPermissions();
    ...
    const fetchRole = async () => {
        try {
            if (!id) return;
            const response = await AdminRolesApiService.getAdminRole(id);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEditMode && id) {
                await AdminRolesApiService.updateAdminRole(id, formData);
                toast.success(t('success'));
            } else {
                await AdminRolesApiService.createAdminRole(formData);
                toast.success(t('success'));
            }

            invalidateCache('admin-roles');
            navigate('/admin-roles');
        } catch (error: any) {
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
                            {isEditMode ? t('editAdminRole') : t('createAdminRole')}
                        </h1>
                    </div>
                </div>
                <button
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800 space-y-6">
                        ...
                        <div className="space-y-1.5">
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">
                                {t('roleName')} <span className="text-rose-500">*</span>
                            </label>
                            <input
                                type="text"
                                required
                        ...
                        <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                            <div>
                                <span className="block text-sm font-semibold text-slate-900 dark:text-white">{t('activeStatus')}</span>
                                <span className="text-xs text-slate-500 dark:text-slate-400">{t('roleActiveDesc')}</span>
                            </div>
