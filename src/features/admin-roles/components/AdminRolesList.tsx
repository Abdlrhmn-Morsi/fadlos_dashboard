...
const AdminRolesList = () => {
    ...
    const fetchRoles = async () => {
        try {
            ...
            const rolesData = Array.isArray(rawData) ? rawData : (rawData.data || []);
    ...
    const confirmDelete = async () => {
        if (!deleteId) return;
        try {
            await AdminRolesApiService.deleteAdminRole(deleteId);
    ...
    return (
        <div className="p-6 space-y-6">
                    ...
                    <table className={clsx("w-full border-collapse", isRTL ? "text-right" : "text-left")}>
                        <thead>
                            <tr className="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
                                <th className="px-6 py-5 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{t('roleName')}</th>
                                <th className="px-6 py-5 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{t('description')}</th>
                                <th className="px-6 py-5 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{t('permissionsTitle')}</th>
            ...
            <ConfirmModal
                isOpen={confirmOpen}
                title={t('deleteAdminRole')}
                message={t('deleteAdminRoleConfirmation')}
